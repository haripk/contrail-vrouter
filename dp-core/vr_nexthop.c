...
static int
nh_l2_rcv(unsigned short vrf, struct vr_packet *pkt,
          struct vr_nexthop *nh, struct vr_forwarding_md *fmd)
{
    struct vr_vrf_stats *stats;
     ...
     */
    if (vr_ip6_neighbor_solicitation(pkt))
        handled = vr_ip6_neighbor_solicitation_input(vrf, pkt, fmd, 0);
    else if (pkt->vp_type == VP_TYPE_IP || pkt->vp_type == VP_TYPE_IP6)
        handled = vr_l3_input(vrf, pkt, fmd);
    else if (pkt->vp_type == VP_TYPE_ARP)
        handled = vr_arp_input(vrf, pkt, fmd, 0);
...
static int
nh_l3_rcv(unsigned short vrf, struct vr_packet *pkt,
        struct vr_nexthop *nh, struct vr_forwarding_md *fmd)
{
    struct vr_vrf_stats *stats;
        ...
        struct vr_nexthop *nh, struct vr_forwarding_md *fmd)
{
    ...
    if (pkt->vp_type == VP_TYPE_ARP || pkt->vp_type == VP_TYPE_IP
        ...
        if (pkt->vp_type == VP_TYPE_ARP) {
            handled = vr_arp_input(vrf, pkt, fmd, pkt_src);
            if (handled)
                return 0;
        } else {
            ...
            if (vr_ip6_neighbor_solicitation(pkt)) {
                handled = vr_ip6_neighbor_solicitation_input(vrf, pkt,
                                                            fmd, pkt_src);
                if (handled)
                    return 0;
            }
...
static int
nh_tunnel_arp_response(unsigned short vrf, struct vr_packet *pkt,
                       struct vr_nexthop *nh, struct vr_forwarding_md *fmd)
{
    struct vr_vrf_stats *stats;
    stats = vr_inet_vrf_stats(vrf, pkt->vp_cpu);
    if (stats)
        stats->vrf_tunnel_arp_responses++;
...
nh_tunnel_add(struct vr_nexthop *nh, vr_nexthop_req *req)
{
    ...
    nh->nh_arp_response = nh_tunnel_arp_response;
...
static int
nh_encap_arp_response(unsigned short vrf, struct vr_packet *pkt,
                      struct vr_nexthop *nh, struct vr_forwarding_md *fmd)
{
    struct vr_vrf_stats *stats;
    stats = vr_inet_vrf_stats(vrf, pkt->vp_cpu);
...
vr_nexthop_add(vr_nexthop_req *req)
{
    ...
    nh->nh_arp_response = nh_drop_arp_response;
